    ...
    if (examConfig.mode === 'template') {
      ...
      return (
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2">总题数</label>
            <input
              type="number"
              min="1"
              max="50"
              ...
              onChange={(e) => {
                ...
                setQuestionTypes({
                  single: Math.floor(total * 0.3),
                  multiple: Math.floor(total * 0.2),
                  fillBlank: Math.floor(total * 0.2),
                  answer: Math.floor(total * 0.2),
                  judge: total - Math.floor(total * 0.3) - Math.floor(total * 0.2) - Math.floor(total * 0.2) - Math.floor(total * 0.2)
                });
              }}
              className="input-field w-full"
              placeholder="请输入总题数"
            />
            <p className="text-xs text-slate-500 mt-2">系统将自动分配题型比例：单选30%、多选20%、填空20%、解答20%、判断10%</p>
          </div>
          <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
            <h4 className="text-sm font-semibold text-slate-700 mb-3">题型分配预览</h4>
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">单选题</span>
                <span className="font-semibold text-slate-900">{questionTypes.single} 道</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">多选题</span>
                <span className="font-semibold text-slate-900">{questionTypes.multiple} 道</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">填空题</span>
                <span className="font-semibold text-slate-900">{questionTypes.fillBlank} 道</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">解答题</span>
                <span className="font-semibold text-slate-900">{questionTypes.answer} 道</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">判断题</span>
                <span className="font-semibold text-slate-900">{questionTypes.judge} 道</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1">
            <div className="card-elevated sticky top-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-slate-900">章节选择</h2>
                <select
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="card-elevated">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-2">出题模式</label>
                  <div className="flex gap-4">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        name="mode"
                        value="template"
                        checked={examConfig.mode === 'template'}
                        onChange={(e) => setExamConfig({...examConfig, mode: e.target.value})}
                        className="w-4 h-4 text-primary-600 focus:ring-primary-500"
                      />
                      <span className="text-sm text-slate-700">模板选题</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="radio"
                        name="mode"
                        value="custom"
                        checked={examConfig.mode === 'custom'}
                        onChange={(e) => setExamConfig({...examConfig, mode: e.target.value})}
                        className="w-4 h-4 text-primary-600 focus:ring-primary-500"
                      />
                      <span className="text-sm text-slate-700">自定义数量</span>
                    </label>
                  </div>
                  <p className="text-xs text-slate-500 mt-2">
                    {examConfig.mode === 'template' ? '按照题型模板设置每种题型的数量' : '自定义总题数，系统自动分配题型'}
                  </p>
                </div>
            ...
            <div className="card-elevated">
              <div className="flex items-center gap-2 mb-6">
                ...
                <span className="text-sm text-slate-500">（{examConfig.mode === 'template' ? '模板选题' : '自定义数量'}）</span>
              </div>

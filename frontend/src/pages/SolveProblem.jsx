...
import { Calculator, Send, Loader, CheckCircle, XCircle, Code, Brain, AlertCircle, Star, BookmarkX } from 'lucide-react';
...
const SolveProblem = () => {
  ...
  const { addToFavorites, addToWrongQuestions, isFavorite, isWrongQuestion } = useFavoriteContext();
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  setShowSuccessMsg('已添加到收藏夹');
                  setTimeout(() => setShowSuccessMsg(''), 2000);
                }}
                disabled={isFavorite(question)}
                className="btn-outline flex items-center disabled:opacity-50 disabled:cursor-not-allowed"
                title="收藏题目"
              >
                <Star className={`w-5 h-5 ${isFavorite(question) ? 'fill-yellow-400 text-yellow-400' : ''}`} />
              </button>
              <button
                onClick={() => {
                  addToWrongQuestions({
                  ...
                  setShowSuccessMsg('已添加到错题本');
                  setTimeout(() => setShowSuccessMsg(''), 2000);
                }}
                disabled={isWrongQuestion(question)}
                className="btn-outline flex items-center disabled:opacity-50 disabled:cursor-not-allowed"
                title="标记为错题"
              >
                <BookmarkX className={`w-5 h-5 ${isWrongQuestion(question) ? 'text-red-500' : ''}`} />
              </button>
            </div>
            {showSuccessMsg && (
              <div className="mt-3 text-sm text-green-600 bg-green-50 border border-green-100 rounded-lg px-3 py-2 text-center animate-slide-up">
                {showSuccessMsg}
              </div>
            )}
          </div>
